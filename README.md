...
- General symbolic exact output normalizes safe logarithm products and powers, odd/even function presentation, integer-`pi` trigonometric shifts, `sin` / `cos` half-`pi` cofunction shifts, and `tan` half-`pi` reciprocal shifts, such as `ln(2*3) = ln(2)+ln(3)`, `ln(sqrt(2)) = 1/2*ln(2)`, `sin(-1) = -sin(1)`, `cos(-1) = cos(1)`, `sin(pi+1/10) = -sin(1/10)`, `cos(pi/2+1/10) = -sin(1/10)`, `tan(pi/2+1/10) = -1/tan(1/10)`, and nested cases like `exp(sin(pi/2+1/10)) = exp(cos(1/10))`.
