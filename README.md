...
- Integer powers and exact rational powers such as `(-8)^(1/3)` and `(-8)^(2/3)` follow the `RealPrincipal` semantics; negative bases with non-rational exponents report `NonRealPower`.
