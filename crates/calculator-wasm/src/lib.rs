...
mod dto_differential {
    ...
    pub const CASES: &[Case] = &[
        ...
        Case {
            id: "symbolic-trig-pi-shift-presentation",
            source: "sin(pi+1/10)",
            request: RequestProfile::ExactOnly,
            expected: ExpectedOutcome::Complete {
                representation: ExactRepresentationKindDto::GeneralSymbolic,
        ...
        Case {
            id: "real-algebraic-nth-root-power",
            source: "(2^(1/3))^(2/5)",
            request: RequestProfile::ExactOnly,
            expected: ExpectedOutcome::Complete {
                representation: ExactRepresentationKindDto::RealAlgebraic,
                plain_text: "(2^(1/3))^(2/5)",
                methods: &[
                    MethodTagDto::AlgebraicMinimalPolynomial,
                    MethodTagDto::AlgebraicRootIsolation,
                    MethodTagDto::CertifiedIntervalEvaluation,
...
mod tests {
    ...
    #[test]
    fn wasm_dto_serializes_translated_real_algebraic_sum() {
        let mut request = exact_only_request();
        request.scientific_output = ScientificOutputRequestDto::Include {
            significant_digits: 50,
            rounding_mode: DecimalRoundingModeDto::NearestTiesToEven,
        };
        let result = calculate_dto("2^(1/3)+1", request);
        let ApiResultDto::Ok {
            value:
                CalculationOutcomeDto::Partial {
                    calculation:
            ...
            panic!("expected partial translated real algebraic calculation");
        };
        assert_eq!(
            exact.representation,
            ExactRepresentationKindDto::RealAlgebraic
        );
        assert_eq!(exact.plain_text, "2^(1/3)+1");
        assert_eq!(
            metadata.exact_representation,
            ExactRepresentationKindDto::RealAlgebraic
        );
    ...
    fn wasm_dto_serializes_inverse_trigonometric_known_values() {
        ...
        let mut degree_request = exact_only_request();
        degree_request.semantics.angle_unit = AngleUnitDto::Degree;
        let result = calculate_dto("atan(sqrt(3))", degree_request);
        let ApiResultDto::Ok {
            value:
                CalculationOutcomeDto::Complete {
                    calculation:
            ...
            panic!("expected radical inverse trig degree calculation");
        };
        assert_eq!(exact.representation, ExactRepresentationKindDto::Integer);
        assert_eq!(exact.plain_text, "60");
        assert_eq!(metadata.semantic_settings.angle_unit, AngleUnitDto::Degree);
        assert!(metadata.methods.contains(&MethodTagDto::SpecialAngle));
    }
    ...
    fn wasm_dto_reports_inverse_trigonometric_out_of_range_error_code() {
        for source in ["asin(2)", "asin(sqrt(2))", "acos(-2)"] {
            assert_eq!(
                calculate_dto(source, exact_only_request()),
                ApiResultDto::Error {
                    error: CalculatorErrorDto::Domain {
...
pub mod wasm_tests {
    ...
    fn wasm32_calculates_inverse_trigonometric_known_values() {
        ...
        degree_request.semantics.angle_unit = AngleUnitDto::Degree;
        assert_eq!(
            exact_plain_text(calculate_dto("atan(sqrt(3))", degree_request)),
            "60"
        );
    }
    ...
    fn wasm32_reports_inverse_trigonometric_out_of_range_error_code() {
        ...
        assert_eq!(
            calculator_error("asin(sqrt(2))", exact_only_request()),
            CalculatorErrorDto::Domain {
                code: DomainErrorCodeDto::InverseTrigonometricOutOfRange,
                span: OptionalTextSpanDto::None,
            }
