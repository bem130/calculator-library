...
and calls the GCD-normalizing constructor. When both canonical denominators are
one, the exact result is already canonical after a single BigInt addition. Wide
integer sums repeatedly pay the unnecessary products, GCD, and exact divisions.
...
- Add a canonical integer/integer path to general `Rational::add`; do not add a
  wide-expression or syntax special case.
...
- Keep mixed and non-integer addition on the existing exact Rational path.
...
- Integer fast-path results match the general canonical constructor for zero,
  signs, cancellation, and large BigInts; mixed/non-integer controls are unchanged.
