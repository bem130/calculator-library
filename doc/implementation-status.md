...
exact dyadic pointのlarge expは、上下で同一のworking precision、certified `ln(2)` pair、midpoint quotient、binary exponentを一度だけ計画する。方向固有のresidual、Taylor bound、dyadic roundingは独立に維持し、非退化endpointは固有precisionのため従来どおり独立計画する。exponent cap、logical-work、no-float、公開protocolを変更しない。
