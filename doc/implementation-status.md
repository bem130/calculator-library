...
exp Taylor recurrenceの共通分母は、一時BigInt `b*n`を構築せず所有する増大値へcanonical入力分母とprimitive項番号を順にin-place乗算する。同じ分母を持つ通常範囲の非退化endpointでは最終分母も共有し、値依存の部分和・項分子、upper tail、異分母・binary-scaling経路は独立に保つ。directed bounds、logical-work、公開protocolは変更しない。
