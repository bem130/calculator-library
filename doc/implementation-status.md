...
* `sinh`、`cosh`、`tanh`、`asinh`、`acosh`、`atanh` は source-level で受け付け、内部 DAG では exp/log/sqrt の組み合わせへ lower する。これにより `sinh(0)=0`、`cosh(0)=1`、`tanh(0)=0`、`asinh(0)=0`、`acosh(1)=0`、`atanh(0)=0`、`exp(sinh(0))=1`、`ln(cosh(0))=0` のような exact simplification が既存の指数・対数簡約経路で処理される。
