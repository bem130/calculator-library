...
* `sinh`、`cosh`、`tanh`、`asinh`、`acosh`、`atanh` は source-level で受け付け、内部 DAG では exp/log/sqrt の組み合わせへ lower する。Add / Multiply のcanonical constructorは平坦化、決定的順序、有理係数、定義済みと証明した同類項をlowering時に統合し、`e^x`、`exp(x)`、`exp(x,e)`を同じ内部表現にする。これにより `sinh(0)=0`、`cosh(0)=1`、`cosh(100)-sinh(100)=exp(-100)`、`cosh(1000)-sinh(1000)-e^(-1000)=0`、`exp(sinh(0))=1`、`ln(cosh(0))=0` のようなexact simplificationが数値評価より前に行われる。相殺と零倍は消える部分式のdomainが証明済みの場合だけ適用する。
...
* resource limit enforcement before or during expensive evaluation paths。再帰的exact normalizationは共有logical-work budgetを使い、代数演算はresultant・factorization・root isolationの保守的上界を開始前に累積予約する。presentation tree と出力文字列にも `max_presentation_nodes` / `max_output_bytes` を適用し、`calculate`、`present`、`presentInput`、生成済みpartial certified enclosureで巨大な出力を返さない。保証区間自体を生成できないpartial outcomeはtyped unavailableと`certifiedEnclosure: null`を返す。
