...
exp Taylor recurrenceのraw分子・正分母が直ちに公開ExactDyadicへ丸められる場合は、canonical RationalのGCD・除算を挟まず同じ比を直接directed roundingする。`0 <= x <= 1`のexact point、非退化endpoint、binary-scaled residualを対象とし、range reduction後の整数冪、負値reciprocal、Rationalを後続処理で使う経路は従来のcanonicalizationを維持する。
