...
exact nonzero central acos pointも正負それぞれのraw directed endpointを使い、paired π enclosureを一度だけ共有する。zero、special、outer、mid-transform fallback、反単調ordering、logical-work、no-float、公開protocolを維持し、paired canonical Rational boundsの正規化を除く。
...
非退化log endpointはprecision-only term planをlower・upper・必要なlog2 compositionで共有する。exact-zero shortcutをplanning前に維持し、独立planning比で対象pathの1,280 bytes / 90 blocksを削減する。
exp finite-sum recurrenceのterm/tail indexと定数2はprimitive integer operandのまま多倍長stateへ乗算し、loopごとのsmall `BigInt`所有化を避ける。general powerと非退化unit expで2,240 bytes / 70 blocksを削減する。
expのceil range reduction divisorはpaired/direct経路ともprimitive scalarとして既存分母へ掛け、reduction 1では入力をborrowする。`exp(2)`で汎用Rational division比40 bytes / 2 blocksを削減する。
