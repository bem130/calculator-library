...
分配で生成するterm数、factor積、sort、merge、新規node見積りは、操作開始前に`max_rewrite_steps`、`max_logical_work_units`、`max_expression_nodes`へ予約する。予約できない場合は部分的に展開せず、元の厳密式を保持してtyped partialへ進む。
