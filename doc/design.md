...
log(x,a) * log(a,b) = log(x,b)
log(x,b) + log(y,b) = log(xy,b)
log(x,b) - log(y,b) = log(x/y,b)
```

`ln(xy) = ln(x) + ln(y)` は、実数モードでは (x>0) かつ (y>0) が証明できる場合だけ適用する。同じく `ln(x/y) = ln(x) - ln(y)` は (x>0) かつ (y>0) が証明できる場合だけ、`ln(x^q) = q ln(x)` は (x>0) と有理数指数 (q) が証明できる場合だけ適用する。

底変換と対数の積・商・和差も、すべての真数が正、すべての底が正かつ1でないことを証明できる場合だけ適用する。商の分母にある対数は真数が1でないことも必要とする。対数連鎖積は各項を `ln(argument) / ln(base)` として収集し、分子・分母を構造的に相殺する。これにより項の順序や二項演算の結合方法に依存せず、完全相殺は1、比が一つ残る場合は単一の `log` へ正規化する。条件を証明できない場合は元のexact symbolic expressionを保持する。
...
一般の四則演算と非負整数冪については、計算用の正本をbounded sparse polynomial formとする。

```text
CanonicalSum = sorted Term[]
Term         = RationalCoefficient * Product(Factor ^ IntegerMultiplicity)
```

`Factor`は、その時点でこれ以上多項式演算として分解しない厳密部分式である。積では同じfactorのmultiplicityを加算し、和では同じfactor mapを持つ項の有理係数を加算する。分配と非負整数冪の展開はこの形式のterm積として行う。このため`x*x`と`x^2`、`a*(b+c)`と`a*b+a*c`は同じ計算正規形になる。商をfactorの負multiplicityへ移すのは、分母と消去対象factorの非零性を証明できる場合だけとする。

自然指数関数はfactor constructorでさらに正規化する。整数`n`について`exp(a)^n = exp(n*a)`とし、同じ積にある自然指数因子は`exp(a)*exp(b) = exp(a+b)`へまとめる。`exp`は常に正なので、商の指数因子も同じ規則で負の引数係数へ移せる。

指数factorの統合後も平方根との合成を一意にするため、実数上で常に成立する`sqrt(exp(a)) = exp(a/2)`をsqrt constructorで適用する。`sqrt(exp(a)^2)`だけを元の構文から特別扱いせず、`exp(2*a)`へfactor統合した後に同じ一般規則で`exp(a)`へ到達する。
...
canonicalizationによって元nodeがaliasまたは別のnormal formへ置き換わる場合、必要な条件を`Defined(expr)`と`NonZero(expr)`のdomain obligationとしてDAGに記録する。normalizationは値の簡約より先にこの集合を検証する。dead nodeが偶然走査されることをdomain safetyの根拠にしてはならない。

lowering後のDAGに対するrecursive exact normalizationは、source calculation arenaでは子を先に確定し、元nodeに対応するside valueへ記録する。bottom-up reduction完了後、非定数のcanonical polynomial viewを表示・後続評価へ渡すため、同じDAGの末尾にcanonical-presentation extension nodeを追加できる。extension内のnodeはchildren-before-parent順であり、追加前にnode数と共有budgetを予約する。先に存在する`Exact` nodeのside presentationが後方のextension rootを参照することは許すが、extension nodeから未正規化source parentへの逆参照とcycleは禁止する。extensionは確定済みexact atomと有理演算だけから構築するため、recursive exact normalizationを再実行しない。

exact normalizationを完了したこのDAGをrational、radical、real algebraic、certified intervalの順に評価する。scientific outputはこのcertified intervalまたはrecognized exact valueから作り、未簡約の元式を別経路で数値評価しない。

子がspecial angle、radical、real algebraic reductionによって後から別のexact valueへ確定した場合、親はそのstored exact valueをatom keyとしてsparse polynomial viewをbottom-upに再構築する。atomの比較は元の`ExprId`だけでなくcanonical exact valueの構造的等値を用いる。したがって`sin(pi/2)*exp(1)-exp(1)`や、異なる構文から同じ実代数的数になったfactorの商も、評価順に依存せず有理数へ縮退できる。
...
limit到達後のdomain検証は、有理演算、stored exact valueの符号、積・商・整数冪のstructural sign、π有理数倍など、既にあるexact情報だけを使う。domain errorを探すために新しいcertified intervalを64 bit等の固定精度で開始してはならない。structural proofで確定できない場合はdomain成功と推測せず、limitを保持したpartialにする。
