...
* **公開契約**: 入力文法、意味論、公開API型、DTO、エラー分類、計算量制限、保証の意味。これらは実装後の互換性契約となる。
...
上の表だけではparserの互換性契約として不足するため、初期版の文法は次で固定する。
...
初期版の関数はすべて単項関数である。`log(2, 10)` のような複数引数、`max` / `min`、ユーザー定義関数は受理しない。
...
exp(0) = 1
log(1) = 0
exp(log(x)) = x      x > 0 が証明済みの場合
log(exp(x)) = x      実数モードの場合
```

`log(xy) = log(x) + log(y)` は、実数モードでは (x>0) かつ (y>0) が証明できる場合だけ適用する。
...
公開APIでは次を用いる。初期版のvariantは一つだけだが、request内に明示して互換性契約にする。
...
| `log([a,b])` | `b <= 0` なら定義域エラー。`a <= 0 < b` の場合は符号判定または精度増加。正と証明できた範囲で `[log_down(a), log_up(b)]` |
...
| `atan([a,b])` | 実数全域で単調増加 |
| `x^y` | 4.7の実数領域規則を先に適用し、一般実数指数では `exp(y log(x))` の区間合成を行う。0や負の底は専用規則で処理する |
...
`ParsedExpression` の内部ASTは公開しない。公開すると構文木構造が互換性契約になり、将来の正規化変更が困難になるためである。
...
### 9.6 protocol versionと互換性

`ProtocolVersion` はDTO互換性のためのversionであり、npm packageのsemverとは別に扱う。

```text
major:
    既存DTOの意味変更、必須field削除、既存codeの意味変更など、
    旧clientが安全に解釈できない変更で増加する。

minor:
    optional field追加、新しいenum/code追加、新しいMethodTag追加など、
    旧clientがunknownとして扱えば安全な変更で増加する。
```

初期公開時のprotocol versionは `1.0` とする。Rust公開enumのうち、利用者が網羅matchし得るものには、必要に応じて `#[non_exhaustive]` を付ける。ただし、計算意味論に関わる `DomainErrorKind`、`DecimalRoundingMode`、`PowerSemantics` は追加時にminor以上のversion更新を必要とする。

TypeScript facadeは、未知の `tag` や未知の `code` を受け取った場合、握りつぶさず `unsupportedProtocol` エラーへ変換する。minor追加に追従していない古いclientでも、誤った成功値として扱ってはならない。
