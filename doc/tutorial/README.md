...
`log(8,2)`、`log(2^(1/3),2)`、`log(8,sqrt(2))`、`ln(e)`、`exp(3,2)` は exact output でそれぞれ `3`、`1/3`、`6`、`1`、`8` になる。底変換と対数法則もcore側で処理され、`ln(8)/ln(2)`、`log(8,3)/log(2,3)`、`log(8,7)*log(7,3)*log(3,2)`、`log(2,10)+log(5,10)` はそれぞれ `3`、`3`、`3`、`1` になる。数値化できない `ln(3)/ln(2)` はexact symbolic expression `log(3,2)` に正規化される。UI側で対数法則を再実装しない。

これらの変形は真数が正、底が正かつ1でないことをcoreが証明できる場合だけ行われる。商の分母にある対数については、真数が1でないことも必要である。条件が未証明なら、推測で変形せず元の式を保持する。
