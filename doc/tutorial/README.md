...
`log(8,2)`、`log(2^(1/3),2)`、`log(8,sqrt(2))`、`ln(e)`、`exp(3,2)` は exact output でそれぞれ `3`、`1/3`、`6`、`1`、`8` になる。`ln(2*3)` は、各 factor の正性を証明できるため `ln(2)+ln(3)` として表示され、`ln(sqrt(2))` は `1/2*ln(2)` として表示される。`sqrt(exp(2)^2)` は、内側が非負であることを証明できるため `exp(2)` として表示される。
